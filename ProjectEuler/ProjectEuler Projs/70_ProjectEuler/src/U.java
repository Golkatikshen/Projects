...
public class U 
{
    ...
    public static Integer[] primeFactorization(int n)
    {
        ...
        Integer[] ints = new Integer[primes.size()];
        return (primes.toArray(ints));
    }
    
    public static int totientEuler(int n)
    {
        Object[] objs = removeRepetitions(primeFactorization(n));
        Integer[] primes = Arrays.copyOf(objs, objs.length, Integer[].class);

        double phi = (1d-(1d/(double)primes[0]));
        for(int i=1; i<primes.length; i++)
            phi *= (1d-(1d/((double)primes[i])));
    ...
    public static Object[] removeRepetitions(Object[] a)
    {
        ArrayList<Object> objs = new ArrayList();
        
        for(int i=0; i<a.length; i++)
            if(!objs.contains(a[i]))
                objs.add(a[i]);
        
        Object[] objss = new Object[objs.size()];
        return objs.toArray(objss);
    ...
    public static boolean isSquare(BigDecimal n)
    {
        BigDecimal nn = sqrtBD(n, 2);
        return (nn.remainder(new BigDecimal("1")).equals(new BigDecimal("0.00")));
    }
    ...
    public static boolean isNumPermutation(String a, String b)
    {
        ...
        for(int i=0; i<a.length(); i++)
        {
            ap[Integer.parseInt(String.valueOf(a.charAt(i)))]++;
            bp[Integer.parseInt(String.valueOf(b.charAt(i)))]++;
        }
    ...
    private static BigDecimal sqrtBI(BigDecimal n, int i) {
