class ANNetwork
{
  ...
  void qLearn(float q_s_a, float reward, float gamma, float[] s_a, float max_q_s_a_p)
  {
    ...
    q_learn_sets.get(q_learn_sets.size()-1)[1] = new float [] {sigmoid(q_s_a + 0.5 * (reward + gamma * max_q_s_a_p - q_s_a), false)};
  }
