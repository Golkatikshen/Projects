class ANNetwork
{
  ...
  void train(float[] target, float[] inputs)
  {
  ...
  //funzioni di implementazione del metodo Q-Learning
  void qLearn(float reward, float gamma, float[] s_a, float max_q_s_a_p)
  {
    //-controllo che la rete abbia solo un neurone di output, altrimenti non è possibile eseguire il Q-Learn
    if(n_outputs > 1)
      return;
      
    q_learn_sets.add(new float[2][]);
    q_learn_sets.get(q_learn_sets.size()-1)[0] = s_a;
    q_learn_sets.get(q_learn_sets.size()-1)[1] = new float [] {sigmoid(reward + gamma * max_q_s_a_p, false)};
  }
  
  float min = 10;
  float error_limit = 0.8;
  void qLearnTrain()
  {    
    boolean again = true;
    ...
    while(again)
    {
      ...
      for(int i=0; i<q_learn_sets.size(); i++)
      {
        ...
        if(total_error > error_limit || total_error < -error_limit)
        {
          if(min > abs(total_error))
          {
            min = abs(total_error);
            println(min);
          }
